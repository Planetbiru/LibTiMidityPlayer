...
static void do_song_load(MidIStream *stream, MidSongOptions *options, MidSong **out, int skip_midi_read)
{
  ...
  *out = NULL;
  ...
  if (! song->oom)
    ...
    DEBUG_MSG("Out of memory while loading MIDI file\n");
...
/* Update the events of a currently playing song with new MIDI data. */
int mid_song_update_events(MidSong *song, MidIStream *stream)
{
    MidEvent *new_events;
    int new_event_count;
    sint32 new_samples;
    ...
    if (song->current_event) {
        current_tick = song->current_event->tick;
    }

    /* Read the new MIDI file into a new event list */
    new_events = read_midi_file(stream, song, &new_event_count, &new_samples);
    if (!new_events) {
        return -2; /* Failed to read new MIDI data */
    }

    /* Free the old events and replace with the new ones */
    timi_free(song->events);
    song->events = new_events;
    song->groomed_event_count = new_event_count;
    song->samples = new_samples;

    /* Load any newly required instruments */
    load_missing_instruments(song);
    if (song->oom) {
        return -3; /* Out of memory loading instruments */
    }

    /* Find the event in the new list that is closest to our old tick */
    song->current_event = song->events; /* Start from the beginning */
    for (i = 0; i < song->groomed_event_count; i++) {
        if (song->events[i].tick >= current_tick) {
            song->current_event = &song->events[i];
            break;
        }
    }
